...
export default {
  commons: {
    ...
    login_username: '账号 或 邮箱',
    input_login_username: '请输入用户 账号 或 邮箱',
    input_name: '请输入名称',
    please_upload: '请上传文件',
  ...
  api_test: {
    ...
    api_import: {
      ...
      import_tip2: '开启URL可重复，接口名称+请求类型+请求路径一致则判断为同一接口',
      import_tip3: '未开启URL可重复，请求类型+请求路径一致则判断为同一接口',
      cover_tip: '覆盖模式',
      cover_tip_1: '1. 如勾选“同步覆盖API模块”选项，则API模块为导入文件中指定的模块',
